...
class operator:
    ...
    def x(self,U):
        ...
        # your code ......
        Ux=0*U
        Ux[1:-2,:]=(-U[0:-3,:]+U[2:-1,:])/(self.grid.hx*2)
        return Ux 
    ...
    def y(self,U):
        # your code ......
        Uy=0*U
        Uy[:,1:-2]=(-U[:,0:-3]+U[:,2:-1])/(self.grid.hy*2)
        return Uy
...
def solveConvectionDiffusionEqn2d(a,b,nu,tf,domain,nx,ny):
    ...
    u0=lambda x,y:np.exp(-40.*((x-0.4)**2+y**2)) # initial condition
    U0=u0(mg.xx,mg.yy)
    ...
    for step in range(numberOfSteps+1):
        ...
        # your code ......
        U[new]=U[cur]+(dt)*(-a*op.x(U[cur]) -b*op.y(U[cur]) +nu*(op.xx(U[cur])+op.yy(U[cur])))
